...
public class RefractionMessagesFabric implements RefractionMessages {
    ...
    public <P extends Packet> void register(Class<P> msgClass, RNetworkDirection direction) {
        if (direction == RNetworkDirection.PLAY_TO_SERVER) {
            ServerPlayNetworking.registerGlobalReceiver(new ResourceLocation(Refraction.MOD_ID, msgClass.getSimpleName().toLowerCase()), (server, player, handler, buf, sender) -> {
                try {
                    P packet = msgClass.getConstructor(FriendlyByteBuf.class).newInstance(buf);
                    packet.handle(player, server::execute);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
        } else {
            ClientPlayNetworking.registerGlobalReceiver(new ResourceLocation(Refraction.MOD_ID, msgClass.getSimpleName().toLowerCase()), (client, handler, buf, responseSender) -> {
                try {
                    P packet = msgClass.getConstructor(FriendlyByteBuf.class).newInstance(buf);
                    packet.handle(ClientData.getPlayer(), client::execute);
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
            });
