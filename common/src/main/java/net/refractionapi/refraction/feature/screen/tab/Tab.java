...
public class Tab {
    ...
    public TabWidget<Checkbox> addCheckbox(Component descriptor, int size, boolean selected, Consumer<Checkbox> consumer) {
        Checkbox checkbox = Checkbox.builder(descriptor, Minecraft.getInstance().font).maxWidth(size).selected(selected).onValueChange((check, newValue) -> consumer.accept(check)).build(); // mid --Zeus
        TabWidget<Checkbox> widget = (TabWidget<Checkbox>) this.createWidget(checkbox);
        widget.hideName();
        return widget;
    }

    public TabWidget<Checkbox> addCheckbox(Component descriptor, int size, boolean selected) {
